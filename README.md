...
FeedbackTab:CreateParagraph({
    Title = "📬 GỬI PHẢN HỒI",
    Content = "Nhập nội dung bạn muốn gửi tới admin. Lưu ý: KHÔNG TROLL!"
})

-- Input box để nhập phản hồi
FeedbackTab:CreateInput({
    Name = "📝 Nội dung phản hồi",
    PlaceholderText = "Viết phản hồi của bạn ở đây...",
    CurrentValue = "",
    Flag = "FeedbackInput",
    Callback = function(Value)
...
FeedbackTab:CreateButton({
    ...
    Callback = function()
        task.spawn(function()
            ...
            local content = feedbackContent or Rayfield:GetValue("FeedbackInput") or ""
            if content == "" then
                Rayfield:Notify({Title = "⚠️", Content = "Vui lòng nhập nội dung trước!", Duration = 3})
                return
            end
            ...
            -- Hàm request (hỗ trợ nhiều executor)
            local requestFunc = (syn and syn.request) or (http and http.request) or request or http_request
            if not requestFunc then
                Rayfield:Notify({Title = "❌", Content = "Executor không hỗ trợ HTTP!", Duration = 4})
                return
            end

            -- Tạo payload
            local payload = {
                ["username"] = "Brainrot Hub Feedback",
                ["embeds"] = {{
                    ...
                    ["fields"] = {
                        {
                            ["name"] = "👤 Người gửi",
                            ["value"] = LocalPlayer.Name .. " (@" .. (LocalPlayer.DisplayName or "") .. ")",
                            ["inline"] = true
                        },
                        {
                            ["name"] = "🆔 User ID",
            ...
            if success and response and (response.StatusCode == 200 or response.StatusCode == 204) then
                Rayfield:Notify({Title = "✅", Content = "Gửi phản hồi thành công!", Duration = 4})
            else
                Rayfield:Notify({Title = "❌", Content = "Gửi thất bại! Kiểm tra webhook.", Duration = 4})
            end
        end)
    end
})
...
FeedbackTab:CreateParagraph({
    ...
    Content = "• Không gửi nội dung troll hoặc spam.\n• Nếu vi phạm, script sẽ tự động cấm/kick bạn nhiều lần.\n• Để muốn hết bị cấm, hãy tìm admin và xin lỗi!\n• Hãy sử dụng tính năng này một cách văn minh."
})

print("✅ Brainrot Hub v7.8 đã sẵn sàng!")
